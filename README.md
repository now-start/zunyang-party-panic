...
생일 방송 30분 전, 플레이어가 이름 없는 백스테이지 조력자가 되어 준비동을 돌아다니며 조사, 세팅, 미니게임, 스토리 시퀀스를 진행하는 libGDX 기반 2D 팬게임입니다.

현재 구조는 `adapter -> application -> domain` 흐름을 기준으로 정리하고 있으며, 허브 이동/상호작용 로직은 use case 중심으로 분리되어 있습니다. 런타임에서는 libGDX `AssetManager`, `InputMultiplexer`, `Scene2D Stage`, `GlyphLayout`, `TiledMap`를 적극 사용하도록 맞춰 두었습니다.
...
게임은 타이틀 화면에서 시작해 준비동 허브를 거쳐 각 챕터로 이동합니다.

현재 문서 기준 서사 구조는 아래 순서를 목표로 합니다.
...
프로토타입 레벨에서는 미니게임형 챕터와 스토리형 챕터가 섞여 있으며, 진행 잠금 해제와 엔딩 톤 계산은 [GameProgress.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/domain/progress/GameProgress.java)가 담당합니다.
...
- `adapter.in`: libGDX 입력, 화면, 렌더링, Scene2D HUD
...
### `org.nowstart.zunyang.partypanic.adapter.in.input`

libGDX `InputProcessor` 계층을 화면별 입력 의도로 변환하는 패키지입니다.

- [MappedActionInputAdapter.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/adapter/in/input/MappedActionInputAdapter.java): 키 코드를 액션 enum으로 매핑하고 중복 keyDown을 억제하는 범용 입력 어댑터입니다.
- [HubInputAdapter.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/adapter/in/input/HubInputAdapter.java): 허브 화면 전용 입력 어댑터입니다. 방향 유지 입력과 확인/뒤로가기 일회성 입력을 분리합니다.

### `org.nowstart.zunyang.partypanic.adapter.in.screen`

libGDX `Screen` 구현체가 모여 있는 인바운드 어댑터 패키지입니다. 사용자의 키 입력을 받고 화면을 렌더링합니다.

- [AbstractGameScreen.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/adapter/in/screen/AbstractGameScreen.java): 공통 `OrthographicCamera`, `FitViewport`, `SpriteBatch`, `InputMultiplexer`, `Scene2D Stage` 생명주기를 담당하는 화면 베이스 클래스입니다.
- [AbstractMiniGameScreen.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/adapter/in/screen/AbstractMiniGameScreen.java): 미니게임 공통 베이스 클래스입니다. 공유 `GameAssets`와 `Scene2dUiFactory`를 받아 배경, 프레임, HUD 렌더링 루프를 제공합니다.
...
- [HubScreen.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/adapter/in/screen/HubScreen.java): 준비동 허브 화면입니다. 플레이어 이동, 조사, 대사 진행, 추천 이벤트 표시, `TiledMap` 기반 허브 타일 렌더링을 담당합니다.
- [PartyPanicScreen.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/adapter/in/screen/PartyPanicScreen.java): 첫 신호 조율 미니게임 화면입니다. `DeskSetupStateMachine`을 사용해 조절형 점수 게임을 렌더링합니다.
...
- [PixelUiRenderer.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/adapter/in/renderer/PixelUiRenderer.java): 패널, 테두리, 텍스트, 텍스처 맞춤 출력 같은 기본 UI 드로잉 도구입니다. 텍스트 폭 계산과 줄바꿈은 libGDX `GlyphLayout`을 사용합니다.
- [MiniGameChrome.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/adapter/in/renderer/MiniGameChrome.java): 미니게임 공통 프레임과 배경을 그리는 정적 유틸리티입니다.
...
- [HubMapRenderer.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/adapter/in/renderer/HubMapRenderer.java): 허브 배경 타일을 programmatic `TiledMap`으로 구성하고, 이벤트/플레이어/위치 패널을 오버레이로 그리는 허브 전용 renderer입니다.

### `org.nowstart.zunyang.partypanic.adapter.in.ui`

미니게임 HUD를 Scene2D 위젯으로 구성하는 패키지입니다.

- [Scene2dUiFactory.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/adapter/in/ui/Scene2dUiFactory.java): `Label`, `ProgressBar`, 패널용 스타일을 공통 생성합니다.
- [PanelTable.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/adapter/in/ui/PanelTable.java): 사각 패널 배경과 테두리를 직접 그리는 Scene2D `Table` 확장입니다.
...
- [DeskSetupStateMachine.java](/Users/moon/IdeaProjects/demo4/src/main/java/org/nowstart/zunyang/partypanic/domain/minigame/DeskSetupStateMachine.java): 첫 신호 조율 미니게임의 선택, 수치 조정, 확인, 시간, 최종 점수를 관리합니다.

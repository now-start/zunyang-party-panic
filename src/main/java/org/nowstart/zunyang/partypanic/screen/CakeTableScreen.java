...
import org.nowstart.zunyang.partypanic.state.CakeBalanceStateMachine;
...
public final class CakeTableScreen extends ScreenAdapter {
    ...
    public CakeTableScreen(PartyPanicGame game, GameProgress progress) {
        this.game = game;
    ...
    private void handleInput() {
        if (Gdx.input.isKeyJustPressed(Input.Keys.ESCAPE)) {
            game.showHub("케이크 테이블에서 허브로 복귀했습니다.");
            return;
        }
        ...
        if (stateMachine.isResult()) {
            ...
            if (Gdx.input.isKeyJustPressed(Input.Keys.H)) {
                game.finishCakeTableMinigame(stateMachine.getFinalScore());
            }
        }
    }
    ...
    private void drawOperatorPanel() {
        ...
        drawLine("최고 점수 " + progress.getBestScore(GameProgress.CAKE_TABLE), left, top - 66f, 0.92f, TEXT_MINT);
